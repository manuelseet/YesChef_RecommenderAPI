...
def obtain_single_recipe_reco(userEmail, max, dbname): 
    ...
    #print(this_user_act)
    this_user_bookmark_list = this_user_act["bookmarks"] ## get List of bookmark objects
    ...
    query_recipeID = this_user_bookmark_list[-1]["recipeId"] #for now, get the latest

    query_recipeID = feature_vectors.index[-1]  ## TEMPORARY OVERRIDE UNTIL BOOKMARK UPDATE TO NEW DB
    ...
    for row in range(test_feature_vector.shape[0]):
        js = jaccard_score(query_feature_vector.iloc[0,:], test_feature_vector.iloc[row,:], average = 'binary')
...
def obtain_you_might_like_reco(userEmail, max, dbname): 
    ...
    query_recipeID = feature_vectors.index[-2]  ## TEMPORARY OVERRIDE UNTIL BOOKMARK UPDATE TO NEW DB
